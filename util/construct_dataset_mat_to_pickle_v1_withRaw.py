...
if version == 'v1':
    # old version 
    input_mat_files_dir = f'F:/dataset/ZuCo/{task_name}/Matlab_files'#修改路径
 
elif version == 'v2':
    # new version, mat73 
    input_mat_files_dir = f'F:/dataset/ZuCo/{task_name}/Matlab_files' #修改路径

output_dir = f'F:/dataset/ZuCo/{task_name}/pickle'#修改路径
...
print(f"找到的文件: {mat_files}")
if len(mat_files) == 0:
    print("未找到任何 .mat 文件，请检查路径是否正确！")
    ...
    for e,sent in enumerate(matdata):
        ...
        if not isinstance(word_data, float):
            ...
            for word in word_data:
                ...
                #GPT修改if条件句
                if hasattr(word, 'nFixations') and isinstance(word.nFixations, (int, np.ndarray)) and np.size(word.nFixations) > 0 and word.nFixations > 0:    

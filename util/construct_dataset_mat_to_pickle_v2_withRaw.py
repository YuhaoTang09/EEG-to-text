...
rootdir = "F:/dataset/ZuCo/task2-NR-2.0/Matlab_files/"
    ...
    if file.endswith(task+".mat"):
        ...
        if subject != 'YMH':
            ...
            for idx in range(len(rawData)):
                ...
                # 添加调试信息
                print(f"当前 wordData 原始内容: {f[wordData[idx][0]]}")
                print(f"提取的 word_data: {word_data}")
                #GPT添加可视化
                print(f"当前句子: {sent_string}")
                '''for widx, data_dict in enumerate(word_data):
                    # 检查 data_dict 是否为字典
                    if not isinstance(data_dict, dict):
                        print(f"单词索引: {widx}, 数据类型不匹配: {type(data_dict)}, 跳过处理。")
                        continue
    
                    print(f"单词索引: {widx}, 内容: {data_dict.get('content', None)}")'''
                #添加到这
                if word_data == {}:
                    ...
                    for widx in range(len(word_data)):
                        ...
                        # 检查单词内容是否有效
                        if 'content' not in data_dict or not data_dict['content'] or len(data_dict['content'].strip()) == 0:
                            print(f"- is not a real word: {data_dict}")
                            continue
    
                        word_obj = {
                            'content': data_dict['content'],
                            'nFixations': data_dict.get('nFix', 0),  # 如果不存在，默认为 0
                            'rawEEG': data_dict.get('RAW_EEG', None)  # 如果不存在，默认为 None
                        }
                    #GPT将以下三行改为上面的
                    #for widx in range(len(word_data)):
                    #    data_dict = word_data[widx]
                    #    word_obj = {'content':data_dict['content'], 'nFixations': data_dict['nFix'], 'rawEEG':data_dict['RAW_EEG']} #@zavidos
                        """if 'rawEEG' in data_dict:
                            word_obj['rawEEG'] = data_dict['rawEEG']
                        else:
                            word_obj['rawEEG'] = None"""

...
ZUCO_SENTIMENT_LABELS = json.load(open('F:/dataset/ZuCo/task1-SR/sentiment_labels/sentiment_labels.json'))
SST_SENTIMENT_LABELS = json.load(open('F:/dataset/stanfordsentiment/ternary_dataset.json'))
...
def get_input_sample(sent_obj, tokenizer, eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], max_len = 56, add_CLS_token = False, subj='unspecified',raw_eeg=False):
    ...
    #完善数据结构
    # 如果不需要情感标签，但仍需要 target_tokenized 的结构
    '''target_tokenized = {'input_ids': torch.zeros((1, max_len), dtype=torch.long),
    ...
    def __init__(self, input_dataset_dicts, phase, tokenizer, subject = 'ALL', eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'],raweeg=False, setting = 'unique_sent', is_add_CLS_token = False):
        ...
        for input_dataset_dict in input_dataset_dicts:
            if subject == 'ALL':
                ...
                # 手动限制仅使用 ZuCo v1.0 的受试者
                subjects = [s for s in input_dataset_dict.keys() if s in zuco_v1_subjects]
...
if __name__ == '__main__':
    ...
    if check_dataset == 'ZuCo':
        ...
        dataset_path_task1 = 'F:/dataset/ZuCo/task1-SR/pickle/task1-SR-dataset.pickle' 
        with open(dataset_path_task1, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))

        dataset_path_task2 = 'F:/dataset/ZuCo/task2-NR/pickle/task2-NR-dataset.pickle' 
        with open(dataset_path_task2, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))

        dataset_path_task3 = 'F:/dataset/ZuCo/task3-TSR/pickle/task3-TSR-dataset.pickle' 
        with open(dataset_path_task2, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
        #修改数据路径
        ...
        for key in whole_dataset_dicts[0]:
            print(f'task3, sentence num in {key}:',len(whole_dataset_dicts[0][key]))
        print()
